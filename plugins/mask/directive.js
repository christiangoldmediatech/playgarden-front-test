...
function getInput (el) {
  if (el.tagName.toLocaleUpperCase() !== 'INPUT') {
    ...
    if (els.length !== 1) { throw new Error(`v-mask precisa de 1 input. Encontrou ${els.length}`) } else { el = els[0] }
  }
  return el
}
...
function getConfig (binding) {
  ...
  if (binding.value && Array.isArray(binding.value) && binding.value.length > 0) { config = binding.value }
  return config
}
