  ...
  computed: {
    ...
    currentWorksheet () {
      if (this.id && this.getLesson) {
        return this.getLesson.worksheets.find(({ id }) => id === this.id)
      }
      return null
    },
    ...
    type () {
      if (this.currentWorksheet) {
        return this.currentWorksheet.worksheetTable.type
      }
      return null
    },
    ...
    question () {
      if (this.currentWorksheet) {
        return this.currentWorksheet.worksheetTable.question
      }
      return ''
    },
    ...
    images () {
      if (this.currentWorksheet) {
        return this.currentWorksheet.worksheetTable.images
      }
      return []
    },

    connectingPairs () {
      if (this.type === 'CONNECTING_PAIRS') {
        // return randomly ordered list of words
        // const test = ['a', 'b', 'c', 'd']
        return shuffle(this.images.map(({ word }) => word))
        // return shuffle(test)
      }
      return null
    },
    ...
    stepCount () {
      if (this.type === 'CONNECTING_PAIRS') {
        return this.connectingPairs.length
      }
      return 1
    },
