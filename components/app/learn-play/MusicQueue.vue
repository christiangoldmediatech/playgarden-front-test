<template>
  <v-menu
    ...
    max-width="1000px"
    @input="handleMenuVisibility"
  >
    <template #activator="{ on }">
      <v-btn
        class="white--text"
        icon
        data-test-id="music-queue-playlist-button"
        v-on="on"
      >
        <v-icon size="32">
    ...
    <v-card class="music-queue" data-test-id="music-queue-card">
      ...
      <v-card-text class="music-queue-content">
        ...
        <template v-else>
          <div
            class="music-queue-empty-queue"
            data-test-id="music-queue-empty-queue"
          >
            Empty queue
          </div>
        </template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const songCountText = computed(() =>
      playlist.value.length === 1 ? '1 song' : `${playlist.value.length} songs`
    )
...
function useVuetifyHelper() { throw new Error('Function not implemented.') }
function useVuetifyHelper() { throw new Error('Function not implemented.') }
